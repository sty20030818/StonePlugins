...
本文件是规则维护与方案选择索引，不是每个工程任务都要加载的方法论清单。普通任务继续按 `SKILL.md` 直接读取匹配的主题 reference；只有任务同时命中多种方法、需要比较迁移策略，或正在维护石头鱼的工程规则时才读取本文件。
...
工程依据的触发与展示格式只由 `SKILL.md` 持有；本索引只保留方法选择、冲突和评测案例，不复制模板或格式契约。
...
| DISCLOSURE-1 | 同一任务中，正确所有者决定修改位置，安全风险又独立决定验证方式 | 在同一个工程依据区块分别说明两项影响，不把它们压成一个空泛方法名 |
| DISCLOSURE-2 | Skill 已注入或 reference 已读取，但方法没有改变最终决定 | 不显示工程依据，不把加载事实冒充采用 |
| DISCLOSURE-3 | KISS、YAGNI 和最小公共面都只导向“不增加无消费者抽象” | 合并为一个通俗的设计依据，不做方法名签到 |
| DISCLOSURE-4 | 一个小型决定由一个准确方法直接决定 | 用单行说明通俗决定、方法名和事实影响，不为套模板强制展开 |
| DISCLOSURE-5 | 安全边界、正确所有者和验证策略分别改变同一任务的不同部分 | 默认双行格式，在一个区块列出多个独立影响并让文字脱离 Emoji 仍可理解 |
| DISCLOSURE-6 | 两个真实方案各有明显收益和代价 | 展开主要收益与未选方案；只有三个以上方案共享比较字段时才用表格 |
| DISCLOSURE-7 | 项目事实决定了选择，但没有准确对应的具名方法 | 标为“工程判断”，不强贴 KISS、DDD、SOLID 等术语 |
