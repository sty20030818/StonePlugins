...
仅当石头鱼的工程规则或按需 reference 实质改变修改所有者、方案范围、抽象或契约、风险边界、兼容策略或验证方式时，最终回复增加 `**🧭 工程依据（石头鱼的工程规则）**`；没有实际影响时不显示。

通常列 1～3 个独立影响。先说人话，再标准确方法：

- `{emoji} **{通俗决定}**（方法论：**{准确名称}**）`

  `{可核查事实或风险}，因此{具体决定或验证}；**结果**：{收益或证据}`

只有一个简单影响且一行能说清时，使用单行列表项。存在真实竞争方案、取舍重要且正文尚未解释时，才在同一项下展开**事实、决定、主要收益、未选方案**及其优缺点；至少三个方案按相同字段比较时才用表格。没有准确具名方法时写 `（**工程判断**）`，不硬贴术语。

Emoji 仅帮助扫读，每项最多一个：🧩 设计/职责、✂️ 简化、🛡️ 安全、🧪 验证、🔄 迁移；文字必须独立表达完整含义。加粗标签的标点放在 `**` 外，例如 `**结果**：`，避免紧贴正文时渲染失败。

同一事实和决定上的相近方法必须合并。仅被注入、读取、与任务相关或事后能够贴名，不算采用；无法说明可核查影响时不显示。这个区块是公开决策摘要，不复述或声称代表内部思维过程。
