...
const FAILURE_CONTEXT = "读取失败则报告并暂停工程决定，不自动安装，也不回退旧缓存。";
const EVENT_CONTEXT = {
    SessionStart: "会话启动、恢复或清空后，工程任务若尚未完整加载 $stonefish-engineering，先通过宿主 Skill 入口读取其 SKILL.md；已加载则沿用，边界不清时加载。",
    SubagentStart: "子 Agent 上下文独立。工程任务须先通过宿主 Skill 入口完整读取 $stonefish-engineering 的 SKILL.md，不得沿用父 Agent 的加载声明。",
    UserPromptSubmit: "工程任务若尚未完整加载 $stonefish-engineering，先通过宿主 Skill 入口读取其 SKILL.md；已加载则沿用，边界不清时加载。",
};
const COMPACT_CONTEXT = "压缩已发生。形成工程决定前，无条件通过宿主 Skill 入口重新完整读取 $stonefish-engineering 的 SKILL.md。";
