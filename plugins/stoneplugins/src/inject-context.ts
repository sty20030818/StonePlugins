...
const FAILURE_CONTEXT =
  "暂停工程决定并报告加载失败，不自动安装，也不回退旧缓存。";

const EVENTS = ["SessionStart", "SubagentStart", "UserPromptSubmit"] as const;
const PROMPT_CONTEXT =
  "工程任务沿用本次完整送达的石头鱼工程核心，按新事实读取细则；核心缺失时通过 $stoneplugins:engineering 完整加载，失败则暂停工程决定。非工程任务遵循个人常驻约定。";

type HookEventName = (typeof EVENTS)[number];

class SafeHookError extends Error {}

function parseHookInput(rawInput: string): HookEventName {
  let input: unknown;
  try {
  ...
  if (typeof event !== "string" || !EVENTS.includes(event as HookEventName)) {
    throw new SafeHookError("Hook 事件不受支持");
  }
  return event as HookEventName;
}

function loadCoreContext(): string {
  ...
  try {
    ...
    if (error instanceof SafeHookError) throw error;
...
let event: HookEventName | undefined;
let output;
try {
