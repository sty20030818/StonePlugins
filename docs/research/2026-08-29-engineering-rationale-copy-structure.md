...
状态：历史研究快照，原位归档。下文的标题候选、格式建议和访谈结论仅对应当时的文案设计，不作为现行模板。现行文档见[文档索引](../README.md)；后续目标与执行边界见 [ADR-0004](../adr/0004-codex-first-bundled-engineering.md) 和[重构计划](../plans/2026-09-14-codex-first-refactor.md)，不代表已实现或验收。
...
> **研究快照**：本文形成于当时的用户确认前。该阶段后续采用过 `## 🧭 本次决策与方法论（石头鱼的工程规则）`，按独立决策展示实际改变用户判断的方法，方案与交付使用不同影响/验证字段；大量决定使用紧凑索引并服从更高优先级的输出约束。这是历史取舍，不是当前标题或模板；现行运行时契约入口见[文档索引](../README.md)。
