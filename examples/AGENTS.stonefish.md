...
- 凡任务涉及代码、配置、依赖、测试、数据、API、迁移、安全、架构、调试、审查或重构，必须应用石头鱼的工程规则；只有不改变技术语义的纯文案或格式编辑可跳过。
- 正常由已信任 Hook 注入核心规则并按条件读取参考文件；Hook 未运行或未验证时，显式使用 `$stonefish-engineering` 并说明降级状态。
