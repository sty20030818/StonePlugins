...
- 凡任务涉及代码、配置、依赖、测试、数据、API、迁移、安全、架构、调试、审查或重构，必须应用 Stonefish Engineering 工作流；只有不改变技术语义的纯文案或格式编辑可跳过。
- 正常由已信任 Hook 注入工作流核心并按条件读取参考文件；Hook 未运行或未验证时，显式使用 `$stonefish-engineering` 并说明降级状态。
- 项目级 `AGENTS.md`、已确认的架构决策、锁文件、根级脚本和仓库惯例优先于个人偏好。只有新建 JS/TS 项目且项目与当前任务都未指定工具链时，才默认 Bun；pnpm 可接受。
