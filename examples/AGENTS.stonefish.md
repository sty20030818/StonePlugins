...
所有任务继续遵守本文件的全局协作规则。需要改变或评价软件行为、代码、配置、依赖、数据、接口、测试、架构或发布时，应用石头鱼的工程规则；边界不清时按工程任务处理。

Codex 使用 `$stoneplugins:engineering`。当前生命周期上下文已完整送达核心时直接沿用，按事实读取细则，不为展示调用重复读核心；核心缺失时通过该入口完整加载。恢复或压缩后以本次完整送达的核心为准，不能沿用旧任务的加载声明。独立 CLI 使用同源 `$engineering` Skill，并在压缩后重新确认完整核心。找不到 Skill 或读取失败时，明确报告并暂停依赖该规则的工程决定，不自动安装或回退旧缓存。
